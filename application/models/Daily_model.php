...
class Daily_model extends CI_Model {
    ...
    function _insert_daily2($dataDaily, $countProid) {
        ...
        for ($i = 0; $i < $countProid; $i++) {
            if ($dataToSql['dr_id'][$i] == "ว่าง") { //ถ้าอเรย์ไหนไม่มี dr_id ให้ทำการ indert
                $query = $this->db->set('daily_dat', $dataToSql['daily_dat'])
                        ->set('daily_start_time', $dataToSql['daily_start_time'][$i])
                        ->set('daily_end_time', $dataToSql['daily_end_time'][$i])
                        ->set('daily_use_time', $dataToSql['daily_use_time'][$i])
                ...
                $dataUpdate = array(
                    'daily_dat' => $dataToSql['daily_dat'],
                    'daily_start_time' => $dataToSql['daily_start_time'][$i],
                    'daily_end_time' => $dataToSql['daily_end_time'][$i],
                    'daily_use_time' => $dataToSql['daily_use_time'][$i],
                    'daily_rec_insert' => $dataToSql['daily_rec_insert'][$i],
                    'daily_note' => $dataToSql['daily_note'][$i]
                );
                $this->db->where('daily_id', $dataToSql['dr_id'][$i]);
                $this->db->update('daily', $dataUpdate);
            }
        }/* .for */
    }
